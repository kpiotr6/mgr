...
def plot_grouped_bars(
    ...
    # Update X-axis labels to include the minutes conversion
    x_labels = [f"{length} steps ({int(length) * 5} min)" for length in output_lengths]
    axis.set_xticklabels(x_labels, rotation=45, ha="right")

    # Clarify the step size on the axis label
    axis.set_xlabel("Output Chunk Length (Step size = 5 min)")
    axis.set_ylabel("MAE")
    if y_axis_max is not None and y_axis_max > 0:
        axis.set_ylim(0, y_axis_max * 1.1)
...
def build_charts(csv_dir: Path, output_dir: Path) -> None:
    ...
    for csv_path in target_files:
        ...
        for input_length in input_lengths:
            ...
            # Align title format with the previous script
            input_minutes = int(input_length) * 5
            title = (
                f"MAE for {target_name}\n"
                f"Input Chunk: {input_length} steps ({input_minutes} min) | Step size = 5 min"
            )

            filename = f"{sanitize_filename(f'MAE_{target_name}_input_{input_length}')}.png"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Plot MAE charts for each target-specific metrics file by input and output chunk length."
    )
    parser.add_argument(
        "--csv-dir",
    ...
    parser.add_argument(
        "--output-dir",
        default="./chart_drawing/MAE_charts/per_target_by_input_output",
