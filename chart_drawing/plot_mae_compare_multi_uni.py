...
def build_charts(csv_all_path: Path, csv_dir: Path, output_dir: Path) -> None:
    ...
    for target_csv in target_files:
        ...
        mae_col_all = f"MAE_{target_name}"

        if mae_col_all not in df_all.columns:
            print(f"Skipping {target_name}, '{mae_col_all}' not found in the all_targets file.")
            continue

        # Determine the metric column in the single target file
        mae_col_single = f"MAE_{target_name}"
        if mae_col_single not in df_single.columns:
            if 'MAE' in df_single.columns:
                mae_col_single = 'MAE'
            else:
        ...
        df_all_target = df_all[['Model', 'InputChunkLength', 'OutputChunkLength', mae_col_all]].copy()
        df_all_target.rename(columns={mae_col_all: 'MAE_Val'}, inplace=True)
        df_all_target['Source'] = 'All Targets'

        # Extract and uniformize Single Target dataframe subset
        df_single_target = df_single[['Model', 'InputChunkLength', 'OutputChunkLength', mae_col_single]].copy()
        df_single_target.rename(columns={mae_col_single: 'MAE_Val'}, inplace=True)
        df_single_target['Source'] = 'Single Target'

        # Combine the sets into one vertical dataframe
        df_combined = pd.concat([df_all_target, df_single_target], ignore_index=True).dropna(subset=['MAE_Val'])
        ...
        for input_length in input_lengths:
            ...
            # Calculate input minutes (1 step = 5 minutes)
            input_minutes = int(input_length) * 5
            ...
            for output_length in output_lengths:
                ...
                # Calculate output minutes (1 step = 5 minutes)
                ...
                pivot = (
                    output_frame.pivot(index="Model", columns="Source", values="MAE_Val")
                ...
                # Updated Title with converted minutes and step size
                title = (
                    f"MAE comparison for {target_name}\n"
                    f"Input: {input_length} steps ({input_minutes} min), "
                    f"Output: {output_length} steps ({output_minutes} min) | Step size = 5 min"
                )

                filename = sanitize_filename(f"mae_compare_{target_name}_in_{input_length}_out_{output_length}.png")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Plot MAE metrics comparison: all targets vs single target models.")
    parser.add_argument(
        "--csv",
    ...
    parser.add_argument(
        "--output-dir",
        default="./chart_drawing/MAE_charts",
