...
def build_charts(csv_path: Path, output_dir: Path) -> None:
    ...
    mape_columns = [
        ...
        if column.startswith("MAPE_") and column != "MAPE"
    ]
    if not mape_columns:
        raise ValueError("CSV does not contain any per-target MAPE columns (expected columns like MAPE_<target>)")

    output_dir.mkdir(parents=True, exist_ok=True)

    input_lengths = sorted(data_frame["InputChunkLength"].dropna().unique())

    for mape_column in mape_columns:
        target_name = mape_column.removeprefix("MAPE_")
        target_frame = data_frame[["Model", "InputChunkLength", "OutputChunkLength", mape_column]].dropna()
        ...
        for input_length in input_lengths:
            ...
            pivot = input_frame.pivot(index="Model", columns="OutputChunkLength", values=mape_column)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Plot MAPE charts for every target and input length in evaluation_metrics_all_targets.csv."
    )
    parser.add_argument(
        "--csv",
    ...
    parser.add_argument(
        "--output-dir",
        default="./chart_drawing/mape_charts/all_targets_by_input_output",
